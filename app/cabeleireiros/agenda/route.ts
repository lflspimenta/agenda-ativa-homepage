...
export async function GET(request: Request) {
  ...
  if (authError || !user?.email) {
    return NextResponse.redirect(
      new URL("/entrar?produto=cabeleireiros", request.url)
    );
  }
  ...
  const { data: buyer, error: buyerError } = await admin
    .from("users")
    .select("first_name, products, cabeleireiros_purchase_date")
  ...
  if (
    ...
    !buyer.products.includes("cabeleireiros")
  ) {
    return NextResponse.redirect(
      new URL("/entrar?estado=sem_acesso&produto=cabeleireiros", request.url)
    );
  }

  const htmlTemplate = await readFile(
    path.join(process.cwd(), "private", "agenda-cabeleireiros-final.html"),
    "utf8"
  );
  const firstName = buyer.first_name?.trim();
  const greeting = firstName ? `Olá, ${escapeHtml(firstName)}` : "Olá";
  const agendaDay = getAgendaDay(buyer.cabeleireiros_purchase_date);
