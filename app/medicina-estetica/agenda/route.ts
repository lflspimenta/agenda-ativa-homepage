...
export async function GET(request: Request) {
  ...
  if (authError || !user?.email) {
    return NextResponse.redirect(
      new URL("/entrar?produto=medicina_estetica", request.url)
    );
  }
  ...
  const { data: buyer, error: buyerError } = await admin
    .from("users")
    .select("first_name, products, medicina_estetica_purchase_date")
  ...
  if (
    ...
    !buyer.products.includes("medicina_estetica")
  ) {
    return NextResponse.redirect(
      new URL(
        "/entrar?estado=sem_acesso&produto=medicina_estetica",
        request.url
      )
    );
  }
  ...
  const htmlTemplate = await readFile(
    path.join(process.cwd(), "private", "agenda-medicina-estetica-final.html"),
    "utf8"
  );
  const firstName = buyer.first_name?.trim();
  const greeting = firstName ? `Olá, ${escapeHtml(firstName)}` : "Olá";
  const agendaDay = getAgendaDay(buyer.medicina_estetica_purchase_date);
