...
export async function GET(request: Request) {
  ...
  if (authError || !user?.email) {
    return NextResponse.redirect(
      new URL("/entrar?produto=fotografos", request.url)
    );
  }
  ...
  const { data: buyer, error: buyerError } = await admin
    .from("users")
    .select("first_name, products, fotografos_purchase_date")
  ...
  if (
    ...
    !buyer.products.includes("fotografos")
  ) {
    return NextResponse.redirect(
  ...
  const htmlTemplate = await readFile(
    path.join(process.cwd(), "private", "agenda-fotografos-final.html"),
    "utf8"
  );
  const firstName = buyer.first_name?.trim();
  const greeting = firstName ? `Olá, ${escapeHtml(firstName)}` : "Olá";
  const agendaDay = getAgendaDay(buyer.fotografos_purchase_date);
