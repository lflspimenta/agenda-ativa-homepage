...
export async function GET(request: Request) {
  ...
  if (authError || !user?.email) {
    return NextResponse.redirect(
      new URL("/entrar?produto=estetica_facial", request.url)
    );
  }
  ...
  const { data: buyer, error: buyerError } = await admin
    .from("users")
    .select("first_name, products, estetica_facial_purchase_date")
  ...
  if (
    ...
    !buyer.products.includes("estetica_facial")
  ) {
    return NextResponse.redirect(
      new URL("/entrar?estado=sem_acesso&produto=estetica_facial", request.url)
    );
  }

  const htmlTemplate = await readFile(
    path.join(process.cwd(), "private", "agenda-estetica-facial-final.html"),
    "utf8"
  );
  const firstName = buyer.first_name?.trim();
  const greeting = firstName ? `Olá, ${escapeHtml(firstName)}` : "Olá";
  const agendaDay = getAgendaDay(buyer.estetica_facial_purchase_date);
