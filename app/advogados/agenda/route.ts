...
export async function GET(request: Request) {
  ...
  if (authError || !user?.email) {
    return NextResponse.redirect(
      new URL("/entrar?produto=advogados", request.url)
    );
  }
  ...
  const { data: buyer, error: buyerError } = await admin
    .from("users")
    .select("first_name, products, advogados_purchase_date")
  ...
  if (
    ...
    !buyer.products.includes("advogados")
  ) {
    return NextResponse.redirect(
      new URL("/entrar?estado=sem_acesso&produto=advogados", request.url)
    );
  }

  const htmlTemplate = await readFile(
    path.join(process.cwd(), "private", "agenda-advogados-final.html"),
    "utf8"
  );
  const firstName = buyer.first_name?.trim();
  const greeting = firstName ? `Olá, ${escapeHtml(firstName)}` : "Olá";
  const agendaDay = getAgendaDay(buyer.advogados_purchase_date);
